...
def bfs(graph, starting_room):
  ...
  while q.size() > 0:
    ...
    # print(path)
    vertex = path[-1]
    if vertex not in inner_visited or new_boolean:
      new_boolean = False
      inner_visited.add(vertex)
      # print(inner_visited)
      dead_end = True
      for destination in graph[vertex]:
        if destination not in inner_visited:
          dead_end = False
      if not dead_end:
        for destination in graph[vertex]:
          if destination not in inner_visited:
      ...
      if dead_end:
        if (len(main_path) == 0):
          ...
          # print('first main', main_path)
          # print('path', path)
          if (main_path[-1] == path[0]):
            main_path.pop()
          main_path += path
        visited.update(path)
        inner_visited.clear()
        ...
        # print(new_path, 'other bfs trigger')
        main_path.pop()
        main_path += new_path
        q = Queue()
        q.enqueue([main_path[-1]])
        new_boolean = True
        # Need to get to first unvisited node
  return main_path

generatedPath = bfs(modifiedMap, player.currentRoom)
# print(generatedPath)
